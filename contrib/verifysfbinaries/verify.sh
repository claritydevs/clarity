...
###   This script attempts to download the signature file SHA256SUMS.asc from clarity.org
...
WORKINGDIR="/tmp/clarity"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://clarity.org/bin/"
VERSIONPREFIX="clarity-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'clarity-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Clarity Core binary release signing key installed?"
   fi
