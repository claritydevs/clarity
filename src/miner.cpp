...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in clarity, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in Clarity
//
...
// ***TODO*** that part changed in clarity, we are using a mix with old one here for now
void static ClarityMiner(const CChainParams& chainparams, CConnman& connman)
{
    LogPrintf("ClarityMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("clarity-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("ClarityMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("ClarityMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("ClarityMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("ClarityMiner -- runtime error: %s\n", e.what());
        return;
    }
}

void GenerateClaritys(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ClarityMiner, boost::cref(chainparams), boost::ref(connman)));
}
