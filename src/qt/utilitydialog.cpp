    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("Clarity Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About Clarity Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  clarity-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_CLARITY_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ClarityGUI::DEFAULT_UIPLATFORM));
        }
...
All the Clarity in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 CLARITY, 0.1 CLARITY, 1 CLARITY and 10 CLARITY -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("Clarity Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(ClarityGUI *window)
{
    if (!window)
        return nullptr;
