...
#include "claritygui.h"

#include "clarityunits.h"
...
const std::string ClarityGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ClarityGUI::DEFAULT_WALLET = "~Default";

ClarityGUI::ClarityGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QString windowTitle = tr("Clarity") + " - ";
#ifdef ENABLE_WALLET
    ...
    QPixmap pixmapLogo(":/images/light/clarity_logo_horizontal");
...
void ClarityGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Clarity address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and clarity: URIs)"));
    ...
    aboutAction = new QAction(tr("&About Clarity"), this);
    aboutAction->setStatusTip(tr("Show information about Clarity"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Clarity"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Clarity addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Clarity addresses"));
    ...
    openAction->setStatusTip(tr("Open a clarity: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Clarity help message to get a list with possible Clarity command-line options"));
...
void ClarityGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void ClarityGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool ClarityGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ClarityGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ClarityGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ClarityGUI::setWalletActionsEnabled(bool enabled)
{
...
void ClarityGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Clarity client") + " " + networkStyle->getTitleAddText();
...
void ClarityGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void ClarityGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ClarityGUI::optionsClicked()
{
...
void ClarityGUI::showDebugWindow()
{
...
void ClarityGUI::showInfo()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_INFO);
}

void ClarityGUI::showConsole()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_CONSOLE);
}

void ClarityGUI::showGraph()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_GRAPH);
}

void ClarityGUI::showPeers()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_PEERS);
}

void ClarityGUI::showRepair()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_REPAIR);
}

void ClarityGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void ClarityGUI::showMNConfEditor()
{
    GUIUtil::openMNConfigfile();
}

void ClarityGUI::showBackups()
{
    GUIUtil::showBackups();
}

void ClarityGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void ClarityGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
void ClarityGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ClarityGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ClarityGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void ClarityGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void ClarityGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void ClarityGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ClarityGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ClarityGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ClarityGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void ClarityGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Clarity network", "", count));
    } else {
...
void ClarityGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ClarityGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void ClarityGUI::updateHeadersSyncProgressLabel()
{
...
void ClarityGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in clarity
        {
...
void ClarityGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void ClarityGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Clarity"); // default title
...
void ClarityGUI::changeEvent(QEvent *e)
{
...
void ClarityGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ClarityGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ClarityGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(ClarityUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void ClarityGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ClarityGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ClarityGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ClarityGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ClarityGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void ClarityGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void ClarityGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ClarityGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ClarityGUI::detectShutdown()
{
...
void ClarityGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ClarityGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ClarityGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ClarityGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ClarityGUI::subscribeToCoreSignals()
{
...
void ClarityGUI::unsubscribeFromCoreSignals()
{
...
void ClarityGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void ClarityGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<ClarityUnits::Unit> units = ClarityUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const ClarityUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(ClarityUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(ClarityUnits::Unit u, ClarityUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(ClarityUnits::name(u)), this);
...
{
    setText(ClarityUnits::name(newUnits));
}
