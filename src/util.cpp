...
const char * const CLARITY_CONF_FILENAME = "clarity.conf";
const char * const CLARITY_PID_FILENAME = "clarityd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "clarity" is a composite category enabling all Clarity-related debug output
            if(ptrCategory->count(string("clarity"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ClarityCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ClarityCore
    // Mac: ~/Library/Application Support/ClarityCore
    // Unix: ~/.claritycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ClarityCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ClarityCore";
#else
    // Unix
    return pathRet / ".claritycore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", CLARITY_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty clarity.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override clarity.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", CLARITY_PID_FILENAME));
